                  ...
                  data: (thread) {
                    return Row(
                      ...
                      children: [
                        IconButton(
                          icon: Icon(
                            thread.isFollowed ? Icons.notifications : Icons.notifications_none,
                          ),
                          tooltip: thread.isFollowed ? l10n.unfollowThread : l10n.followThread,
                          onPressed: () async {
                            ...
                            result.fold(
                              ...
                              (_) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      thread.isFollowed ? l10n.unfollowedThread : l10n.followingThread,
                                    ),
                                  ),
                                );
                              },
                            ...
                            result.fold(
                              ...
                              (_) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      thread.isMuted ? 'Thread unmuted' : 'Thread muted',
                                    ),
                                  ),
                                );
                              },
              ...
              data: (thread) {
                ...
                return postsAsync.when(
                  data: (posts) {
                    ...
                    if (_previousPostCount != null && currentPostCount > _previousPostCount!) {
                      ...
                      if (_previousPostCount! > 0) {
                        WidgetsBinding.instance.addPostFrameCallback((_) {
                          unawaited(
                            ref.read(communityMutationsProvider.notifier).markThreadRead(widget.threadId),
                          );
                        });
                      }
                    }
                  ...
                  error: (error, stackTrace) => ErrorView.inline(
                    ...
                    onRetry: () => ref.invalidate(threadPostsProvider(widget.threadId)),
                  ),
                );
              },
              loading: () => const Center(child: AdisuProgressIndicator()),
