// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, unused_import, invalid_annotation_target, unnecessary_import

import 'package:freezed_annotation/freezed_annotation.dart';

part 'update_step_dependency.freezed.dart';
part 'update_step_dependency.g.dart';

@Freezed()
abstract class UpdateStepDependency with _$UpdateStepDependency {
  const factory UpdateStepDependency({
    /// Dependency type
    required String dependencyType,

    /// Required step ID
    required String requiredStepId,
  }) = _UpdateStepDependency;

  factory UpdateStepDependency.fromJson(Map<String, Object?> json) => _$UpdateStepDependencyFromJson(json);
}
