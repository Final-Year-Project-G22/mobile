// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, unused_import, invalid_annotation_target, unnecessary_import

import 'package:freezed_annotation/freezed_annotation.dart';

part 'register_device_response_body.freezed.dart';
part 'register_device_response_body.g.dart';

@Freezed()
abstract class RegisterDeviceResponseBody with _$RegisterDeviceResponseBody {
  const factory RegisterDeviceResponseBody({
    /// Device ID
    required String id,

    /// Success message
    required String message,

    /// A URL to the JSON Schema for this object.
    @JsonKey(name: '\$schema') String? schema,
  }) = _RegisterDeviceResponseBody;

  factory RegisterDeviceResponseBody.fromJson(Map<String, Object?> json) => _$RegisterDeviceResponseBodyFromJson(json);
}
